...
import { formatedDate, inwordEnglish, numberWithComma, sortArray, titleCamelCase } from "@/lib/utils";
...
const Moneyreceipt = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            try {
                ...
                const [responsePayment, responseCustomer] = await Promise.all([
                    getDataFromFirebase("payment", userId),
                    getDataFromFirebase("customer", userId)
                ]);
               
                const result = responsePayment.map(payment =>{
                    const matchCustomer = responseCustomer.find(customer => customer.id === payment.customerId);
                    return{
                        ...payment,
                        matchCustomer
                    }
    ...
    const printHandler = (id) => {
        try {
            ...
            doc.addImage(`/images/moneyreceipt/${data.cashType === 'Cash' ? 'cash.png' : 'cheque.png'}`, "PNG", 0, 0, 210, 297);
            ...
            doc.text(`${data.matchCustomer.name}`, 120, 74, { align: "center" });
            const total = inwordEnglish(data.amount);

            doc.text(`${titleCamelCase(total)} Only`, 109, 80.25, { align: "center" });
            doc.text(`${data.cashType === 'Cash' ? '' : data.bank}`, 121, 87, { align: "center" });
            doc.text(`${data.cashType === 'Cash' ? '' : formatedDate(data.chequeDt)}`, 164.5, 87, { align: "left" });

            doc.text(`${data.purpose}`, 92, 93, { align: "center" });
            doc.text(`${data.matchCustomer.phone}`, 165, 93, { align: "center" });
                ...
                <table className="w-full border border-gray-200">
                    <thead>
                        <tr className="w-full bg-gray-200">
                            ...
                            <th className="flex justify-end border-b border-gray-200 px-4 py-1">Action
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        {moneyreceipts.length ? (
                            moneyreceipts.map(moneyreceipt => (
                                <tr className="border-b border-gray-200 hover:bg-gray-100" key={moneyreceipt.id}>
                                    ...
                                    <td className="text-start py-1 px-4">{moneyreceipt.matchCustomer.name}</td>
                                    <td className="text-center py-1 px-4">{moneyreceipt.amount}</td>
                                    <td className="text-center py-1 px-4">{moneyreceipt.cashType}</td>
                                    <td className="text-center py-1 px-4">{moneyreceipt.matchCustomer.phone}</td>
                                    <td className="text-center py-2">
                                        <div className="h-8 flex justify-end items-center space-x-1 mt-1 mr-6">

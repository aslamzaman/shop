...
import { BtnSubmit, DropdownEn, TextDt, TextNum, TextEnDisabled, TextEn } from "@/components/Form";
...
const Edit = ({ message, id, data }) => {
    ...
    const showEditForm = async () => {
        ...
        try {
            ...
            const { customerId, dt, cashType, amount, remarks, yr } = data;
    ...
    const saveHandler = async (e) => {
        ...
        try {
            setBusy(true);
            // 7 objects ------
            const arrayObject = [customerId, dt, cashType, amount, remarks, yr];
                        ...
                        <div className="px-4 pb-6 text-black">
                            <form onSubmit={saveHandler} >
                                <div className="grid grid-cols-1 gap-2 my-4">
                                    ...
                                    <DropdownEn Title="Cash Type" Id="cashType" Change={e => setCashType(e.target.value)} Value={cashType}>
                                        <option value="Cash">Cash</option>
                                        <option value="Cheque">Cheque</option>
                                    </DropdownEn>

                                    <TextNum Title="Amount" Id="amount" Change={e => setAmount(e.target.value)} Value={amount} />
                                    <TextEn Title="Remarks" Id="remarks" Change={e => setRemarks(e.target.value)} Value={remarks} />

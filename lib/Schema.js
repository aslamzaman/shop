...
export const purchaseSchema = (data = []) => {
    ...
    const [dt, shipment, vendorId, productId, shadeNo, qty, price, yr] = data;  // shadeNo === Thann
    return {
        dt: dt.toString(),
        shipment: shipment.toString(),
        vendorId: vendorId.toString(),
        productId: productId.toString(),
        shadeNo: Number(shadeNo),   
        qty: Number(qty),
        price: Number(price),
        yr: Number(yr),
        createdAt: new Date().toISOString()
...
export const paymentSchema = (data = []) => {
    if (!Array.isArray(data) || data.length < 6) {
        throw new Error("Data array of at least 6 elements");
    }
...
export const invoicePDFSchema = (data = []) => {
    if (!Array.isArray(data) || data.length < 7) {
        throw new Error("Data array of at least 7 elements");
    }
    const [invoiceNo, dt, customerId, payment, deduct, yr, products] = data;

    const sanitizedProducts = Array.isArray(products)
        ? products.map(item => {
            let productItem = item.productId;
            return {
                productId: productItem.toString(),
                thn: Number(item.thn),
                qty: Number(item.qty),
